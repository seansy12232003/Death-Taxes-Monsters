...
func _ready():
	print(self.get_path())
	# set stats
	level = $"../../../".get_node("KnightBoss").level
	Game.dataBaseMonsters[5]["Defense"] = Game.calculate_defense(level) # Defense
	print("Defense " + str(Game.dataBaseMonsters[5]["Defense"]))
	
	#Set attack power of every attack depending on stats
	for i in Game.dataBaseMonsters[5]["Attacks"]:
		var path = "../BattleUI/Fight/GridContainer/Attack " + str(((i) + 1))
		Game.dataBaseMonsters[5]["Attacks"][i]["Damage"] = Game.calculate_attack_power(Game.dataBaseMonsters[5]["Attacks"][i]["Damage"], level)
		print(Game.dataBaseMonsters[5]["Attacks"][i]["Name"])
		print(Game.dataBaseMonsters[5]["Attacks"][i]["Damage"])
